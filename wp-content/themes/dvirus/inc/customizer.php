<?php
/**
 * dvirus Customizer functionality
 *
 * @package WordPress
 ...
 */
function dvirus_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'dvirus_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'dvirus' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => dvirus_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'dvirus' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'dvirus' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'dvirus' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'dvirus' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'dvirus' );
}
add_action( 'customize_register', 'dvirus_customize_register', 11 );

/**
 * Register color schemes for dvirus.
 *
 * Can be filtered with {@see 'dvirus_color_schemes'}.
 *
 ...
 */
function dvirus_get_color_schemes() {
	return apply_filters( 'dvirus_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'dvirus' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'dvirus_get_color_scheme' ) ) :
/**
 * Get the current dvirus color scheme.
 *
 * @since dvirus 1.0
 *
 * @return array An associative array of either the current or default color scheme hex values.
 */
function dvirus_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = dvirus_get_color_schemes();
...
endif; // dvirus_get_color_scheme

if ( ! function_exists( 'dvirus_get_color_scheme_choices' ) ) :
/**
 * Returns an array of color scheme choices registered for dvirus.
 *
 * @since dvirus 1.0
 *
 * @return array Array of color schemes.
 */
function dvirus_get_color_scheme_choices() {
	$color_schemes                = dvirus_get_color_schemes();
...
endif; // dvirus_get_color_scheme_choices

if ( ! function_exists( 'dvirus_sanitize_color_scheme' ) ) :
/**
 ...
 */
function dvirus_sanitize_color_scheme( $value ) {
	$color_schemes = dvirus_get_color_scheme_choices();
...
endif; // dvirus_sanitize_color_scheme
 ...
 */
function dvirus_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = dvirus_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = dvirus_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = dvirus_get_color_scheme_css( $colors );

	wp_add_inline_style( 'dvirus-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'dvirus_color_scheme_css' );
 ...
 * @since dvirus 1.0
 */
function dvirus_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', dvirus_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'dvirus_customize_control_js' );
 ...
 * @since dvirus 1.0
 */
function dvirus_customize_preview_js() {
	wp_enqueue_script( 'dvirus-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'dvirus_customize_preview_js' );
 ...
 */
function dvirus_get_color_scheme_css( $colors ) {
 ...
 * @since dvirus 1.0
 */
function dvirus_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-dvirus-color-scheme">
		<?php echo dvirus_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'dvirus_color_scheme_css_template' );
